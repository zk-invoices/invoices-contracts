...
describe('InvoicesProgram', () => {
    it('test basic functions', async () => {
        ...
        const proof1 = await InvoicesProgram.setLimit(proof.publicInput, proof, limitAmount, Signature.create(timeAuthorityKey, proof.publicOutput.nonce.toFields().concat(proof.publicInput.creditAccount.toFields()).concat(limitAmount.toFields())));
        console.timeEnd('setLimit');
        console.time('create');
        const createProof = await InvoicesProgram.createInvoice(proof.publicInput, proof1, invc, new InvoicesWitness(tree.getWitness(0n)), createSign, actionTimestamp);
        ...
        const claimProof = await InvoicesProgram.claimInvoice(proof.publicInput, proof1, invc, new InvoicesWitness(tree.getWitness(0n)), claimSign, claimActionTimestamp);
        console.time('claim');
        console.time('verify');
...
//# sourceMappingURL=InvoicesProgram.test.js.map