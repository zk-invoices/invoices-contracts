...
// external API
export { LocalStore, PersistentMerkleTree, BaseMerkleWitness };
// internal API
export { maybeSwap };
class LocalStore {
    constructor() {
     ...
     */
    async getNode(level, index, _default) {
        return this.nodes[level]?.[index.toString()] ?? _default;
    }
    // TODO: this allows to set a node at an index larger than the size. OK?
    async setNode(level, index, value) {
        var _a;
        return ((_a = this.nodes)[level] ?? (_a[level] = {}))[index.toString()] = value;
    }
}
/**
...
class BaseMerkleWitness {
     ...
     */
    constructor(witness, _height) {
        let height = witness.length + 1;
        if (height !== _height) {
            throw Error(`Length of witness ${height}-1 doesn't match static tree height ${_height}.`);
        }
        this.height = _height;
